...
public class Bank {
     ...
     */
    public ArrayList<Account> getbAccounts(){
        return this.BANKACCOUNTS;
    }

    public <T> void showAccounts(Class<T> accountType){
        if(this.getbAccounts().size() < 1){
            System.out.println("There are no accounts in this Bank!");
            return;
        }
        for(Account account : this.getbAccounts()){
    ...
    public Account getBankAccount(Bank bank, String accountNum){
        Account acc = null;
        for(Account account: bank.getbAccounts()){
    ...
    public ArrayList<Field<String, ?>> createNewAccount(){
        ...
        //ACCOUNTNUMBER
        Field<String, Integer> accountNumber = new Field<String,Integer>("ACCOUNTNUMBER",String.class,8, new Field.StringFieldLengthValidator());
        ...
        //OWNERFNAME
        Field<String, Integer> ownerFName = new Field<String,Integer>("OWNERFNAME",String.class,3, new Field.StringFieldLengthValidator());
        ownerFName.setFieldValue("Enter Owner First Name: ",false);
        arrayOfBasicInfo.add(ownerFName);

        //OWNERLNAME
        Field<String, String> ownerLName = new Field<String,String>("OWNERLNAME", String.class , "", new Field.StringFieldValidator());
        ownerLName.setFieldValue("Enter Owner Last Name: ", false);
        arrayOfBasicInfo.add(ownerLName);

        //OWNEREMAIL
        Field<String, String> ownerEmail = new Field<String,String>("OWNEREMAIL", String.class , "", new Field.StringFieldValidator());
...
//        Field <Double, Double> creditLimit = new Field<Double, Double>("creditLimit",Double.class,1000.00,new Field.DoubleFieldValidator());
//        creditLimit.setFieldValue("");
    ...
    public SavingsAccount createNewSavingsAccount() {
        ArrayList<Field<String, ?>> arrayOfBasicInfo = this.createNewAccount();
        Field<Double, Double> initialDeposit = new Field<Double, Double>("initialDeposit",Double.class,1.00,new Field.DoubleFieldValidator());
        do{
            initialDeposit.setFieldValue("Enter initial deposit: ");
            if(initialDeposit.getFieldValue() > this.getDepositLimit()){
                System.out.println("Initial deposit must not be greater than the Banks " + this.getDepositLimit() + " deposit limit.");
            }
    ...
    public void addNewAccount(Account account){
        if(!accountExists(this, account.getACCOUNTNUMBER())){
            this.getbAccounts().add(account);
        }
        else{
            System.out.println("Account with account#"+account.getACCOUNTNUMBER() + " already exists! \n=====Account Creation Aborted!=====");
        }
    }
    ...
    public static class BankCredentialsComparator implements Comparator<Bank>{
        //A comparator that compares if two bank objects have the same set of credentials.
        @Override
        public int compare(Bank b1, Bank b2) {

            if(b1.getName().strip().equalsIgnoreCase(b2.getName().strip()) && b1.getPasscode().strip().equals(b2.getPasscode().strip())){
